...
unsigned int nTargetSpacing = 1 * 90; // 1.5 minute blocks
...
const string strMessageMagic = "MOXEN Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    if(pindexBest->nHeight < 51)
    {
        ...
        else if(pindexBest->nHeight < 501)
    {
        nSubsidy = 1000 * COIN;
    }
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    // proof of stake rewards. POS begins at block 501
    
    int64_t nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8);  //default 5% yr

        if(pindexBest->nHeight < 501)
    {
        nSubsidy = 25 * COIN;
    }
        else if(pindexBest->nHeight < 1001)
        ...
        else if(pindexBest->nHeight < 2001)
    {
        ...
        else if(pindexBest->nHeight < 3001)
    {
        nSubsidy = 50 * COIN;
    }
        else if(pindexBest->nHeight < 4001)
    {
        nSubsidy = 25 * COIN;
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "MOXEN", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "05 Jun 2015";
        CTransaction txNew;
        txNew.nTime = 1433772301;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1433772301;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 31627747;
       ...
       if (false && (block.GetHash() != hashGenesisBlock)) {
        ...
        assert(block.hashMerkleRoot == uint256("0x7eb25eadf3657b998965e7e37451cf3cd4c76d5ff267e3a16a3d3f061ebb93cf"));
